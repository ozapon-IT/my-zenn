...
API のホストは IPv4/IPv6 の両方で名前解決できます(2026年8月時点)。
...
なお公式ドキュメントには IP 制限機能そのものの説明(「会員ページにてIP制限機能を有効にすると、API呼出ができるIPアドレスを制限することができます」)はありますが、IPv4 と IPv6 の別への言及はありません(2026年8月時点、全文検索で確認)。
...
認証を通過したあとに `ifoOrder` の数量を JSON number で送ると `ERR-5105` になる話は [ERR-5105の記事](https://zenn.dev/ozapon/articles/gmo-fx-err5105-ifo-size) に書いています。
