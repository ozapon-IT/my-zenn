...
- 当日の分足リプレイと **分類が1件でもずれたら halt** し、新規の仮想約定を止めます。ずれを残したまま走ると、記録そのものが証拠にならないためです
...
- **halt**: 日次検算で分類が不一致になったとき、翌セッションから新規の仮想約定を止めるフラグ
...
3. **日次検算**が独立した分足リプレイと分類を突合する。`mismatch` なら halt
4. **週次集計**が稼働率と推定滑りの分布を出す。halt 中なら継続日数も出す。サマリに載せるのは執行品質だけです
...
抜粋は簡略化しています。終端では no-op にし、開場でない気配は状態を進めません。
...
誤認がまずいのは、稼働率の数え方です。機会損失に計上するのは障害(`system_outage`)だけで、閉場は別理由です。API エラーを閉場に落とすと、その日が無通知で消えます。検算側も、klines の業務エラーでは halt せず `defer` に倒します。比較材料が無いのに不一致を出さない、という fail-closed です。
...
- 既に halt 中の再通知 → しない。解除するまで毎夕鳴ると、安全弁がノイズになります

休場は実測で踏んでいます。カレンダー上は休みでも、取引所の分足が残ることがあります。リプレイは普通に判定を出し、デモは見送りにしています。この日を突合すると偽の不一致で halt するので、見送り理由を突合対象から外しました。

分類が合っていても、開始バッチが置いたトリガー価格がリプレイと違えば halt します。これはデータ経路のバグ(確定足フィルタや日付の切り出し)を想定しています。到達時刻がリプレイのバーに入るかは記録のみです。ポーリング解像度の問題であって、分類の誤りではないためです。
...
halt 中は開始バッチが `SKIPPED(halted)` を書いて終わります。解除は原因を直してから手動です。日次が再通知しない副作用として、halt の継続は Slack の日次からは分かりません。週次サマリに halt の有無と継続日数を必ず載せ、解除忘れをそこで検知します。

### 合否は執行品質だけ見る

週次が見るのは次です。閾値超過は警告だけで、halt はしません。止めるのは日次の分類不一致だけです。
...
Public REST の呼び出し上限は、公式ドキュメントに明記がありません(2026年8月時点。明記があるのは Private GET / POST と WebSocket の subscribe)。下限1秒、既定5秒にし、throttle を観測したら間隔を倍化します。既存クライアントの GET リミッターも併用します。レート制御の本体は[クライアント側で強制する記事](https://zenn.dev/ozapon/articles/gmo-fx-rate-limiter)です。
...
halt せず、週次でまとめて判断する案です。運用は楽になります。しかし不一致の日が記録に混ざると、一致率も滑りも事後に信頼できません。デモの目的がパイプラインの正しさなら、ずれた時点で止める方が一貫します。価格の差や欠測は halt しない、と対象を狭くしたのは、この厳しさを保つためです。
...
- テスト環境のない本番 API で執行を観測するなら、**発注しない経路**を先に作ります。根拠はゲートの閉じではなく、Public GET だけの参照と、IAM が取引シークレットを持たないことです
- 仮想ブローカーは観測列で状態を進めます。判定コアを I/O から切り離すと、日次のリプレイ検算が同じ関数で書けます
- 開場でない気配は捨てます。閉場の ticker は成功の形で返り、統計だけを静かに汚します
- 分類の不一致は1件で halt します。価格差・欠測・見送り日は halt しません。安全弁が毎夕鳴らないことと、ずれを記録に混ぜないことを両立させるためです
- 合否は執行品質だけです。週次サマリにもトレードの結果は載せません

予約型の系統で、発注する価格を誰が決めるかは[LLMに損切り価格を出させない設計](https://zenn.dev/ozapon/articles/gmo-fx-llm-sl-python)に書いています。本記事は発注しない観測側です。誤発注の封じ込めは[テスト環境のない本番APIで誤発注を封じ込める設計](https://zenn.dev/ozapon/articles/gmo-fx-order-containment)、閉場判定は[閉場なのに建玉取得が通った話](https://zenn.dev/ozapon/articles/gmo-fx-market-status-closed)です。
