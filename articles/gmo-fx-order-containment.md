...
無効ボディでの確認と、最小ロット疎通を分けることで、「仕様を知るために連続稼働の封じ込めを緩める」ことを避けています。
...
そこで段階を分けました。まず無効ボディで業務エラーを誘発し、必須パラメータ名など注文を成立させなくても分かる情報を確定する。成功レスポンスの形(例えば `data` が注文オブジェクトの配列であること)は、その後の制御下の最小ロット疎通で確認しました。封じ込めを外したまま連続稼働で仕様を漁る、という進め方は採っていません。
