...
- プロセス内のリミッターで済ませられるのは、**定時バッチ同士が時間差の cron で重ならないアーキテクチャ**とセットの判断です。プロセスを跨いで重なり得る唯一の例外(緊急停止)は、レートリミットではなく冪等設計で安全側に倒します

## 前提: システム概要と制約条件

GMOコインFX の API を使う自動売買システム(AWS Lambda + Python)です。GMO を呼ぶのは、EventBridge cron で起動される2つの定時バッチ——**判断バッチ**(発注判断と IFD-OCO 発注、平日 20:30 JST)と**手仕舞いバッチ**(建玉の強制決済、火〜土 4:30 JST)——に加えて、緊急時にダッシュボードから手動で全建玉を成行決済する **Kill Switch Lambda** です。

制約条件は次のとおりです。

- GMO Private API の呼び出し上限は **POST 1回/秒・GET 6回/秒**。なお公式ドキュメントには「制限はシステム負荷状況によって一時的に変更する場合があります」とあり、上限ぴったりを狙わず余裕を持って守る前提です
...
GET(残高照会・建玉一覧など)は失敗してもリトライすればよく、in-flight 直列化までは不要です。`Lock` + 最小間隔(≥167ms)のシンプルな実装にし、POST とはカウンタを独立させています。Public エンドポイント(価格取得等)の GET にも同じ間隔を適用しています。
...
プロセス内リミッターは、Lambda インスタンスを跨いでは効きません。それでも分散化しなかったのは、**実行が重なり得るケースを絞り込めている**からです。定時バッチ同士は起動時間帯が異なり(20:30 と 4:30)、cron 起動のみなので重なりません。

唯一の例外が手動トリガーの Kill Switch で、手仕舞いバッチと同時に走る可能性があります。ここは**レートリミットの保証対象にせず、同時発火しても冪等になるよう決済処理側で吸収する**設計です(仕様書にもこの前提を明記しています)。瞬間的にはアカウント単位の 1回/秒を超え得ますが、その帰結はレート超過エラーであって二重注文ではありません。緊急停止という役割上、稀な超過エラーの可能性より「確実に止まること」を優先しました。
