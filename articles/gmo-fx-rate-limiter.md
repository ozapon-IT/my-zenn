...
- プロセス内のリミッターで済ませられるのは、**同時実行が構造的に起きないアーキテクチャ**(EventBridge の時間差 cron 起動)とセットの判断です

## 前提: システム概要と制約条件

GMOコインFX の API を使う自動売買システム(AWS Lambda + Python)です。GMO を呼ぶのは2つの定時バッチだけで、**判断バッチ**(発注判断と IFD-OCO 発注、平日 20:30 JST)と**手仕舞いバッチ**(建玉の強制決済、火〜土 4:30 JST)が EventBridge cron で起動されます。

制約条件は次のとおりです。

- GMO Private API の呼び出し上限は **POST 1回/秒・GET 6回/秒**(公式仕様)
...
```mermaid
sequenceDiagram
    participant C as GmoFxClient
    participant R as request_with_retry
    participant L as RateLimiter
    participant G as GMO FX API

    C->>R: private_post(path, body)
    R->>L: wait_post()
    Note over L: in-flight 解消を待つ +<br/>前回応答から ≥1.0秒 を保証
    R->>G: POST(署名ヘッダ付き)
    G-->>R: レスポンス
    R->>L: mark_post_done()(finally で必ず実行)
    Note over L: クールダウン起点を「いま」に更新
```

`wait_post()` は 401 リトライの**各試行前**に呼ばれるフック(`pre_attempt_hook`)として渡してあり、初回もリトライも同じ制御を通ります。
...
GET(残高照会・建玉一覧など)は失敗してもリトライすればよく、in-flight 直列化までは不要です。`Lock` + 最小間隔(≥167ms)のシンプルな実装にし、POST とはカウンタを独立させています。
...
プロセス内リミッターは、Lambda が並列実行されれば効きません。それでも分散化しなかったのは、**そもそも GMO を呼ぶ実行が同時に2つ存在しない**アーキテクチャだからです。GMO を呼ぶのは時間帯の異なる2つの cron バッチのみで、API Gateway 起点などの並列トリガーはありません。

この判断は前提とセットです。もし「ユーザー操作起点で GMO を叩く」ような構成に変えるなら、この前提が崩れるため分散リミッターが必要になります。制約はコードのコメントではなくアーキテクチャ文書側に明記しています。
...
- プロセス内リミッターで済むかは**同時実行が起きないアーキテクチャかどうか**とセットで判断する

同じシステムの認証まわりでハマった話は [GMOコインFX APIのERR-5010でハマった話 — 署名対象パスとAcceptヘッダの2つの罠](https://zenn.dev/ozapon/articles/gmo-fx-hmac-sign-path) に書いています。
