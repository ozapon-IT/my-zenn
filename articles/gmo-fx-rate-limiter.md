...
唯一の例外が手動トリガーの非常停止で、定時バッチと同時に走る可能性があります。ここは**レートリミットの保証対象にせず、同時発火しても冪等になるよう決済処理側で吸収する**設計です(詳細は[非常停止(Kill Switch)の冪等設計](https://zenn.dev/ozapon/articles/gmo-fx-kill-switch-idempotency)に書きました)。瞬間的にはアカウント単位の 1回/秒を超え得ますが、その帰結はレート超過エラーであって二重注文ではありません。非常停止という役割上、稀な超過エラーの可能性より「確実に止まること」を優先しました。
