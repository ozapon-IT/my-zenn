...
- プロセス内のリミッターで済ませられるのは、**発注・決済の Private POST を出す定時同士が時間差で重ならない**こととセットの判断です。プロセスを跨いでそれが重なり得る例外(非常停止)は、レートリミットではなく冪等設計で安全側に倒します
...
GET は上限も失敗時のコストも別なので、プロセスを跨いだ間隔の保証は対象外です。
...
発注系 API は失敗時の再送判断が高くつくので、サーバに弾かれる前にクライアントで送り出しを止めます。計測タイミングが不明なら、クールダウンは保守側に倒します。
