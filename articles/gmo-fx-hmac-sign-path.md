...
[公式ドキュメント](https://api.coin.z.com/fxdocs/)の ERR-5010 の説明は「リクエストヘッダーに指定されているAPI-SIGN(Signature)に不正がある場合に返ってきます」であり、`Accept` への言及はドキュメントにありません(2026年8月時点)。そのため、このケースではエラーコードから真因にたどり着くのが難しくなります。
