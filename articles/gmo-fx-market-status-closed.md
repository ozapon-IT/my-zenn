...
代理判定が検知できるのは、実際には「API が壊れている」ことだけでした。閉場という状態は、参照系 GET の応答のどこにも現れません。自分は「参照が通る」を「取引できる」と読み替えていましたが、この2つの間に含意関係はありませんでした。

Step0 が素通りすると、シーケンスはそのまま取消・決済という**発注系 POST に進みます**。今回のスパイクは GET のみなので閉場中の POST 応答は採っていませんが、少なくとも「閉場エラーで失敗し得る経路に入る」ことは避けたい状態です。
...
そのうえで、旧来の参照系 GET の健全性チェックは**捨てずに残します**。こちらは閉場の検知には使えませんでしたが、「市場は開いているが API 側が壊れている」ケースの検知としては引き続き有効だからです。**2つを両方満たしたときだけ**続行します。
...
- 閉場中でも Private 参照系 GET は HTTP 200 + `status: 0` を返す。**参照が通ることは開場の証拠にならない**
- 市場の開閉は `GET /public/v1/status` の **`data.status`** で判定する。`OPEN` 完全一致のときだけ続行し、それ以外・不明・取得失敗はすべて閉場扱いに倒す
- ボディ外側の `status`(API 成否)と `data.status`(市場状態)は同名で別物。どちらを見ているかを意識する
- 参照系 GET の健全性チェックは「API 障害」の検知として残す価値がある。**閉場の検知と API 障害の検知は別の目的**なので、片方でもう片方を兼ねさせない
- 手仕舞いや非常停止の先頭に開閉確認を置くなら、代理判定ではなく公式ステータスを主判定にする
